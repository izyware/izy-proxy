...
module.exports = (function() {
  ...
  modtask.create = function(queryObject, logOverrideFn) {
    ...
    var ret  = function(numNewSamplesRecieved) {
      ...
      rateTracker.timeElapsed = now - rateTracker.firstSampleTS;

      if (rateTracker.timeElapsed > rateTracker.recalcWindowMS) {
        rateTracker.firstSampleTS = null;
        rateTracker.accumulatedDataSize = 0;
        rateTracker.windowSize = 0;
        rateTracker.timeElapsed = 0;
      }
      if (rateTracker.accumulatedDataSize == 0) {
        rateTracker.firstSampleTS = now;
      };


      rateTracker.windowSize++;
      totalDataSize += step;
      rateTracker.accumulatedDataSize += step;
      totalBufferBeforeLogging += step;
      if (rateTracker.timeElapsed > rateTracker.recalcWindowMS / 2) {
        samplesPerSecond = rateTracker.accumulatedDataSize * 8 / (rateTracker.timeElapsed / 1000) / bitsPerSample ;
      }
            ...
            for(var p in msg) {
              switch(p) {
                ...
                case 'errorObject':
                  ...
                  outcomeMonitoring = { reason: 'An errorObject is present and should be in the monitoring data' };
                  break;
                case 'audioSource':
                ...
                case 'method':
                  ...
                  break;
                case 'event':
                  actionString = msg[p] + '#' + actionString;
                  break;
                case 'action':
                  actionString = actionString + msg[p];
                  break;
                default:
