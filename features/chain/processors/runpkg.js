...
var modtask = function(chainItem, cb, chainProcessorSystem) {
    if (chainItem.method.indexOf('//') == 0) {
        var str = chainItem.method;
        var i = 1;
        var queryObject = chainItem.udt[i++];
        modtask.doLaunchString(chainProcessorSystem, str, queryObject, function(outcome) {
            var context = chainItem.udt[i++] || chainItem.chainContext;
...
modtask.doLaunchString = function(chainProcessorSystem, launchString, payload, cb) {
    var apiGatewayUrls = {
        'inline': 'inline',
    ...
    if (url == 'inline') {
        return modtask.handlers.inline(chainProcessorSystem, cb, parsedLaunchString, payload);
    }
    return modtask.handlers.http(cb, parsedLaunchString, payload, url);
}

modtask.handlers = {};
modtask.handlers.inline = function(chainProcessorSystem, cb, parsedLaunchString, payload) {
    var parsed = modtask.ldmod('kernel/path').parseInvokeString(parsedLaunchString.invokeString);
    var runModule = function() {
        try {
            var mod = modtask.ldmod(parsed.mod).sp('doChain', chainProcessorSystem.doChain).processQueries(payload, cb);
        } catch (e) {
            return cb({
                reason: e.message
            });
